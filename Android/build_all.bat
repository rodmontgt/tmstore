...
for /f %%f in ('dir /b /AD') do (
	...
	if exist "%cd%/%%f/dump" (
		echo ## DUMP directory found for [%cd%/%%f]. no need to create new dump ##
	) else (
		java -jar -Dfile.encoding=UTF-8 "%TOOLS_DIRECTORY%\APKBiilder3.jar" "%SETUP_DIRECTORY%" "%cd%/%%f"
		REM echo sdk.dir=%ANDROID_SDK_PATH% > "%%f/dump/local.properties"
		REM echo "lalalalala" >> "%%f/dump/local.properties"
		for /f %%g in ('dir "local.properties" /b/s') do (
			echo sdk.dir=%ANDROID_SDK_PATH%>"%%g"
		)
	)
	cd "%cd%/%%f/dump"
	call compile_release.bat
	REM call gradlew.bat assembleRelease
	REM call gradlew.bat assembleCommonRelease
	REM call gradlew.bat assembleAutoCompleteGeoLocationRelease
	REM call gradlew.bat assemblePayPalRelease
	REM copy /Y "%%f/dump/*.apk" "%%f"
	copy /Y "*.apk" "../"
	cd..
	cd..
)
