...
@interface ViewControllerCategoriesNew () {
...
@implementation ViewControllerCategoriesNew
...
- (void)viewDidLoad {
    ...
    if (dm.isPriceFilterLoaded  && dm.isAtributtFilterLoaded && dm.enable_filters ) {
        ...
        [self setFilterVisibilityNew:true];
        ...
        if(_viewUserDefined[_kShowAllItems]) {
            _viewUserDefined[_kShowAllItems].frame = CGRectMake(
                                       0,
                                       0,
                                       _scrollView.frame.size.width,
                                       _scrollView.frame.size.height - 20
                                       );
        }
#endif
        ...
        [self setFilterVisibilityNew:YES];
...
- (IBAction)barButtonBackPressed:(id)sender {
    ...
    ViewControllerCategoriesNew* vcCategories = nil;
    if([self.parentViewController isKindOfClass:[ViewControllerCategoriesNew class]]){
        vcCategories = (ViewControllerCategoriesNew*)(self.parentViewController);
    }
...
#pragma mark - Methods
- (int)getChildCount {
    int childCount = _currentItem.childCount;
    NSLog(@"_childCount %d",childCount);
    if (self.showFilterdResult) {
        childCount = (int)[[ProductInfo getFilteredItems] count];
        NSLog(@"getFilteredItems_childCount %d",childCount);
    }
    return childCount;
}
...
#if CATEGORY_VIEW_NEW_HACK_ENABLE
    [_scrollViewBase setScrollEnabled:false];
    [_scrollViewChild setScrollEnabled:true];
#endif
...
#if ENABLE_HEADER_FILTER_BUTTON
        customFilterButton.hidden = false;
         [self setFilterVisibilityNew:false];
#else
        _filterView.hidden = false;
        _scrollView.frame = CGRectMake(
                                       _scrollView.frame.origin.x,
...
#if CATEGORY_VIEW_NEW_HACK_ENABLE
    if ([imageArray count] > 0 && [[Addons sharedManager] show_category_banner]&& 0) 
#else
...
- (void)resetMainScrollView {
    ...
    [_scrollView setContentSize:CGSizeMake(_scrollView.contentSize.width, globalPosY)];
//    [_scrollViewBase.layer setBorderWidth:1];
//    PRINT_SIZE_STR(@"_scrollView.contentSize", _scrollView.contentSize);
//    PRINT_POINT_STR(@"_scrollView.contentOffset", _scrollView.contentOffset);
    if (_scrollViewBase) {
//        RLOG_DESC(@"view.frame.size.height = %.2f", self.view.frame.size.height);
//        RLOG_DESC(@"CGRectGetMaxY(_scrollViewBase.frame) = %.2f", CGRectGetMaxY(_scrollViewBase.frame));
        
        float diff = self.view.frame.size.height - CGRectGetMaxY(_scrollViewBase.frame);
        if(diff < 20){
            CGRect sRect = _scrollViewBase.frame;
            sRect.size.height -= (20 - diff);
            _scrollViewBase.frame = sRect;
        }
    }
    if (_scrollViewChild) {
//        RLOG_DESC(@"view.frame.size.height = %.2f", _scrollViewBase.frame.size.height);
//        RLOG_DESC(@"CGRectGetMaxY(_scrollViewChild.frame) = %.2f", CGRectGetMaxY(_scrollViewChild.frame));
        
        float diff = _scrollViewBase.frame.size.height - CGRectGetMaxY(_scrollViewChild.frame);
        if(diff < 20){
            CGRect sRect = _scrollViewChild.frame;
            sRect.size.height -= (20 - diff);
            _scrollViewChild.frame = sRect;
        }
    }
...
- (void)clickOnCategory:(CategoryInfo*)categoryClicked currentItemData:(DataPass*)currentItemData{
    ...
    ViewControllerCategoriesNew* vcCategories = [[Utility sharedManager] pushScreen:mainVC.vcCenterTop];
...
//    [_scrollViewBase setBackgroundColor:[UIColor yellowColor]];
    ...
    if (isFirstTimeSpinner) {
        [_spinnerView setCenter:CGPointMake(self.view.frame.size.width/2, [_scrollView contentSize].height - _spinnerView.frame.size.height * 2)];
    }
    [_spinnerView startAnimating];
}
...
-(void)getAttributeWithID {
    ...
    RLOG(@"*****************TM_ProductFilter***********%@",pfObj);
    for (TM_FilterAttribute* faObj in pfObj.getAttributes) {
        RLOG(@"################ TM_FilterAttribute ################%@",faObj);
...
-(BOOL)containsFilterAttribute :(TM_FilterAttribute*)filterAttribute1{
    ...
    for (TM_FilterAttribute* filterAttribute2 in filterAttributes) {
        if ([filterAttribute1.title isEqualToString:filterAttribute2.title]) {
            return true;
        }
    }
    return false;
