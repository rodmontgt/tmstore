...
#import "Utility.h"
#define GOOGLE_MOBILE_VISION_BARCODE_SCANNER 0
#define MTB_BARCODE_SCANNER 1
#if !(TARGET_OS_SIMULATOR) && ENABLE_BARCODE_SCANNER && GOOGLE_MOBILE_VISION_BARCODE_SCANNER
@import GoogleMobileVision;
#import "DrawingUtility.h"
@interface BarcodeScannerViewController ()<AVCaptureVideoDataOutputSampleBufferDelegate> {
  ...
  dispatch_sync(dispatch_get_main_queue(), ^{
    ...
    for (GMVBarcodeFeature *barcode in barcodes) {
      ...
      [self.overlayView addSubview:label];
        ...
        //rishabh
        if ((unsigned long)barcodes.count > 0) {
            for (GMVBarcodeFeature* obj in barcodes) {
                NSLog(@"obj = %@", obj);
                [self barcodeRawValue:obj.rawValue];
                break;
            }
            return;
        }
...
@end
#elif !(TARGET_OS_SIMULATOR) && ENABLE_BARCODE_SCANNER && MTB_BARCODE_SCANNER
