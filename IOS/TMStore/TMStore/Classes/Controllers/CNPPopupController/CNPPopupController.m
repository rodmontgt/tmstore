...
- (instancetype)initWithContents:(NSArray *)contents {
    ...
    if (self) {
        ...
        _VIEW_POPUP = self.popupView;
        self.popupView.backgroundColor = [Utility getUIColor:kUIColorBgTheme];
        self.popupView.clipsToBounds = YES;
        
        self.maskView = [[UIView alloc] initWithFrame:self.applicationWindow.bounds];
        _VIEW_MASK = self.maskView;
...
- (void)applyTheme {
    ...
    if (self.theme.popupStyle == CNPPopupStyleFullscreen) {
        ...
        if (self.theme.themeId == 2) {
            maskBackgroundColor = self.theme.maskType == CNPPopupMaskTypeClear?[UIColor clearColor] : [UIColor colorWithWhite:0.0 alpha:0.9f];
        }
    }
...
- (CGSize)calculateContentSizeThatFits:(CGSize)size andUpdateLayout:(BOOL)update
{
    ...
    if(self.theme.themeId == 1) {
        //address theme
        result.width =  self.theme.size.width;//[[MyDevice sharedManager] screenSize].width - [[MyDevice sharedManager] screenSize].width * 0.1f;
        result.height = self.theme.size.height;//[[MyDevice sharedManager] screenSize].height - [[MyDevice sharedManager] screenSize].width * 0.1f;
    }else {
        //default theme
...
- (void)presentPopupControllerAnimated:(BOOL)flag parentView:(UIView*)parentView {
...
- (void)handleBackgroundTapGesture:(id)sender {
    if (self.theme.shouldDismissOnBackgroundTouch) {
        [self.delegate backgroundTouchEventRegistered:self];
...
+ (CNPPopupTheme *)consentScreenTheme {
    CNPPopupTheme *defaultTheme = [[CNPPopupTheme alloc] init];
    defaultTheme.themeId = 1;
    defaultTheme.backgroundColor = [UIColor whiteColor];
    defaultTheme.cornerRadius = 0.0f;
    ...
    defaultTheme.maxPopupWidth = [[MyDevice sharedManager] screenSize].width * 1.0f;
    return defaultTheme;
}
+ (CNPPopupTheme *)addressTheme {
    CNPPopupTheme *defaultTheme = [[CNPPopupTheme alloc] init];
    defaultTheme.themeId = 1;
    defaultTheme.backgroundColor = [UIColor whiteColor];
    ...
    defaultTheme.movesAboveKeyboard = YES;
    defaultTheme.contentVerticalPadding = 16.0f;
    defaultTheme.maxPopupWidth = [[MyDevice sharedManager] screenSize].width * 0.90f;
    return defaultTheme;
}
+ (CNPPopupTheme *)zoomTheme {
    CNPPopupTheme *defaultTheme = [[CNPPopupTheme alloc] init];
    defaultTheme.themeId = 2;
    defaultTheme.backgroundColor = [UIColor clearColor];
    defaultTheme.cornerRadius = 0;
