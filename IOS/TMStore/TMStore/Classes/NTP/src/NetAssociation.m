...
//        RLOG(@"Unable to start listening on socket for [%@] due to error [%@]", server, error);
        return;
    }
}
...
//    RLOG(@"[%@] index=%i {%5.3f,%5.3f,%5.3f,%5.3f,%5.3f,%5.3f,%5.3f,%5.3f}", server, fifoIndex % 8, fifoQueue[0], fifoQueue[1], fifoQueue[2], fifoQueue[3], fifoQueue[4], fifoQueue[5], fifoQueue[6], fifoQueue[7]);
/*┌──────────────────────────────────────────────────────────────────────────────────────────────────┐
...
//        RLOG(@"[%@] index=%i {good: %i; fail: %i; none: %i} offset=%3.1f", server, fifoIndex, good, fail, none, offset * 1000.0);
...
//            RLOG(@"[%@] poll interval adusted: %3.1f >> %3.1f", server, [repeatingTimer timeInterval], pollIntervals[pollingIntervalIndex]);
            [repeatingTimer invalidate];
            [repeatingTimer release];
...
/*┌──────────────────────────────────────────────────────────────────────────────────────────────────┐
  ...
  └──────────────────────────────────────────────────────────────────────────────────────────────────┘*/
    ...
    if ( !success ) {
//        RLOG(@"Unable to get system uptime");
        return;
    }
...
//    RLOG(@"Socket failed to connect to [%@] due to error : [%@]", server, error);            
}

- (void) udpSocket:(GCDAsyncUdpSocket *)sock didNotSendDataWithTag:(long)tag dueToError:(NSError *)error {
//    RLOG(@"Socket failed to send to [%@] due to error : [%@]", server, error);        
}

- (void) udpSocketDidClose:(GCDAsyncUdpSocket *)sock withError:(NSError *)error {
//    RLOG(@"Socket closed : [%@] error : [%@]", server, error);    
}
...
/* ----------------------------------------------------------------------------------------------------
 RLOG(@"Read SUCCESS: [%@] clock offset: %5.3fs±%5.3fmS (%5.3f,%5.3f,%5.3f))", server, offset, dispersion, (offset-offset2)*1000.0, (offset-offset3)*1000.0, (offset-offset4)*1000.0);

 RLOG(@"Read SUCCESS: [%@] (%i : %3.1fs))", server, stratum, - [[self  dateFromNetworkTime:&ntpServerBaseTime] timeIntervalSinceNow]);

 RLOG(@"Read SUCCESS: [%@] clock offset: %5.3fs±%5.3fmS", server, offset, dispersion);

 RLOG(@"Read SUCCESS: [%@] - %@ - offset: %5.3fs±%5.3fmS (%i - %i/%i)", server, trusty ? @"GOOD" : @"FAIL", offset, dispersion, [fifoQueue count], answerCount, trustyCount);
*/
