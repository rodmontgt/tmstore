...
- (UIActivityIndicatorView *)createSmallDefaultActivityIndicatorView {
    ...
    smallDefaultActivityIndicatorView.activityIndicatorViewStyle = UIActivityIndicatorViewStyleWhiteLarge;//rishabh
...
- (void)show:(BOOL)animated {
    ...
    if (animated) {
        //        [self setSubviewTransform:CGAffineTransformMakeScale(1.3f, 1.3f) alpha:0.5f];// rishabh
        [self setSubviewTransform:CGAffineTransformMakeScale(1.03f, 1.0f) alpha:0.5f];
    ...
    //rishabh
    if (self.mode == MRProgressOverlayViewModeIndeterminateSmallDefault) {
        self.blurView.alpha = 0.0f;
        self.backgroundColor = [UIColor colorWithWhite:0 alpha:0.4f];
        //        CGAffineTransform transform = CGAffineTransformMakeScale(1.5f, 1.5f);
        //        self.modeView.transform = transform;
    }
    //rishabh
}
...
- (void)hide:(BOOL)animated completion:(void(^)())completionBlock {
    ...
    void(^animBlock)() = ^{
        //        [self setSubviewTransform:CGAffineTransformMakeScale(0.6f, 0.6f) alpha:0.0f];//rishabh
        [self setSubviewTransform:CGAffineTransformMakeScale(1.0f, 1.0f) alpha:0.0f];
...
- (void)manualLayoutSubviews {
    ...
    self.center = CGPointMake((bounds.size.width - insets.left - insets.right) / 2.0f, (bounds.size.height - insets.top - insets.bottom) / 2.0f);
    ...
    if (self.isMannualPositionEnable) {
        self.center = self.mannualPosition;
        self.bounds = self.mannualBound;
    }
    ...
    if (!self.titleLabel.hidden && isTextNonEmpty) {
        ...
        CGRect boundingRect = [self.titleLabel.attributedText boundingRectWithSize:titleLabelMaxSize options:NSStringDrawingUsesLineFragmentOrigin context:nil];
